...
const httpServer = http.createServer((req, res) => {
  // Serve sempre index.html per qualsiasi richiesta alla root
  const filePath = path.join(__dirname, 'index.html');
  fs.readFile(filePath, (err, data) => {
    if (err) {
      res.writeHead(404);
      res.end('index.html not found - ' + filePath);
      return;
    }
...
const TILE = 32;
// Spawn in open floor areas (tile 0), away from walls
// TILE=32, spawning at tile coords 3-5 (blue, top-left open) and 23-25 (red, bottom-right open)
const SPAWN_BLUE = [
  {x:3*TILE+16,y:3*TILE+16},{x:4*TILE+16,y:3*TILE+16},
  {x:3*TILE+16,y:4*TILE+16},{x:4*TILE+16,y:4*TILE+16},
  {x:5*TILE+16,y:3*TILE+16},{x:5*TILE+16,y:4*TILE+16}
];
const SPAWN_RED = [
  {x:25*TILE+16,y:19*TILE+16},{x:24*TILE+16,y:19*TILE+16},
  {x:25*TILE+16,y:18*TILE+16},{x:24*TILE+16,y:18*TILE+16},
  {x:23*TILE+16,y:19*TILE+16},{x:23*TILE+16,y:18*TILE+16}
];
